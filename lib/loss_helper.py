...
def compute_aux_situation_loss(data_dict, situation_loss_tag):
    if '__class__' in situation_loss_tag:
        loss_pos = F.cross_entropy(data_dict["aux_scores"][:, :, 0], data_dict["auxiliary_task_loc_gt"])
        if '__l2__' in situation_loss_tag:
            loss_rot = F.mse_loss(data_dict["aux_scores"][:, :, 1:], data_dict["auxiliary_task_rot_gt"], reduction="mean")
        elif '__l1__' in situation_loss_tag:
            loss_rot = F.l1_loss(data_dict["aux_scores"][:, :, 1:], data_dict["auxiliary_task_rot_gt"], reduction="mean")
    else:
        if '__l2__' in situation_loss_tag:
            loss_pos = F.mse_loss(data_dict["aux_scores"][:, :3], data_dict["auxiliary_task"][:, :3], reduction="mean")
            loss_rot = F.mse_loss(data_dict["aux_scores"][:, 3:], data_dict["auxiliary_task"][:, 3:], reduction="mean")
        elif '__l1__' in situation_loss_tag:
            loss_pos = F.l1_loss(data_dict["aux_scores"][:, :3], data_dict["auxiliary_task"][:, :3], reduction="mean")
            loss_rot = F.l1_loss(data_dict["aux_scores"][:, 3:], data_dict["auxiliary_task"][:, 3:], reduction="mean")
    loss_aux = CONF.LOSS.SITUATION_POS_W * loss_pos + CONF.LOSS.SITUATION_ROT_W * loss_rot
    return loss_aux, loss_pos, loss_rot
...
def get_loss(data_dict, config, situation_loss_tag, detection=False, use_aux_situation=False, use_answer=True, ):
    """ Loss functions
    ...
    if use_aux_situation:
        aux_loss, loss_position, loss_quaternion = compute_aux_situation_loss(data_dict, situation_loss_tag)
    ...
    loss =    CONF.LOSS.VOTE_W * data_dict['vote_loss'] \
            + CONF.LOSS.OBJECTNESS_W * data_dict['objectness_loss'] \
            + CONF.LOSS.BOX_W * data_dict['box_loss'] \
            + CONF.LOSS.SEM_CLS_W * data_dict['sem_cls_loss'] \
            + CONF.LOSS.SITUATION_W * data_dict["aux_loss"] \
            + CONF.LOSS.QA_W * data_dict['answer_loss']
