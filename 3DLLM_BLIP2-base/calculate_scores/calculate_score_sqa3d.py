...
QT=['All', 'What', 'How', 'Can', 'Is', 'Which', 'Other']
...
if __name__=="__main__":
    ...
    golds = json.load(open('/scratch/bbsg/yunzem2/dataset/sqa3d/SQA3D/ScanQA/data/qa/SQA_test.json'))

    e = args.epoch
    apreds = []
    
    all_eval_json_files = [file for file in os.listdir("%s/result/"%args.folder) if file.startswith("test_%d_vqa_result_rank"%e)]
    all_eval_json_files = sorted(all_eval_json_files, key = lambda x: (int(x.replace("test_%d_vqa_result_rank"%e, "").replace(".json", "")), x))
    ...
    pc_feat_root = "/scratch/bbsg/yunzem2/dataset/3DLLM/finetuning_scannet/voxelized_features_sam_nonzero_preprocess"
    ...
    print (len(golds), len(list(preds.keys())))
    ...
    for (q,g) in enumerate(golds):
        ...
        preds_[qclass1(g['question'])][q]=preds[q]
